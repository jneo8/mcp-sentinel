...
def test_cli_run_once(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    config_text = dedent(
        """
        sentinel:
          incident-cards:
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["--config", str(config_path), "run", "--run-once"])
...
def test_cli_missing_config(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["--config", str(missing_path), "run", "--run-once"])
